...
from curve_eval import curve_eval_regression, curve_eval_classification, curve_predict
...
def CurveLossmesh(curve, N_points = 11, x_min = -.2, x_max = 1.2, test_loader=None, loss_fn=None, device="cpu", logger_info=None, verbose=False, model_maker=None):
    ...
    model_mesh = model_maker().to(device)  # creates a new empty model
    num_obs = N_points**2
...
def bezier_plot(curve, device, test_loader, plottype="semilog", N_bezierpoints = 20, logger_info=print, 
    ...
    if classification_task:
        ...
        curve_eval = curve_eval_regression
    if eval_results is None:
        ts = torch.linspace(0, 1, N_bezierpoints, device=device)
        curve_perpoint_measurement_dict, ts, curve_ensemble_measurement_dict = curve_eval(curve, ts=ts,
        ...
        if plot_linear:
            line_perpoint_measurement_dict, _, line_ensemble_measurement_dict = curve_eval(curve,ts=ts,
                                                                    test_loader=test_loader, device=device, 
